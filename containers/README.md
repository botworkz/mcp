...
The `+mcp-echo-image` target name and `botwork/mcp-echo:local` tag are a stable contract: sibling/local builds in `botworkz/space` and `botworkz/vm` consume this target via `FROM ../mcp+mcp-echo-image`.
